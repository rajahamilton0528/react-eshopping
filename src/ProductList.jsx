...
import { addItem, removeItem } from './CartSlice'; // Import addItem and removeItem from CartSlice
import './ProductList.css';

function ProductList() {
    const [showCart, setShowCart] = useState(false); 
    const [addedToCart, setAddedToCart] = useState({}); // State to track which items are added to cart
    const dispatch = useDispatch();

    // Get total items and cart items from Redux
    ...
    // Filter non-numeric characters from cost
    ...
    const handleRemoveFromCart = (product) => {
        ...
        // Set the item as not in the cart
        setAddedToCart((prevState) => ({
            ...prevState,
            [product.name]: false, // Mark product as removed
        }));
    };

    // Sync button state when items are added or removed from the cart in Redux
    useEffect(() => {
        const updatedCartStatus = {};
        cartItems.forEach((item) => {
            updatedCartStatus[item.name] = true; // Mark as added
        });
    ...
    const handlePlantsClick = (e) => {
        e.preventDefault();
        setShowCart(false); // Hide the cart and show plants when clicked
    };
                ...
                <div style={styleObjUl}>
                    ...
                    <div>
                        <a href="#" onClick={handleCartClick} style={styleA}>
                            <h2 className="cart">
                                <div className="product-cart-count cart_quantity_count">{totalItems}</div>
            ...
            {!showCart ? (
                <div className="product-grid">
                    {plantsArray.map((category, index) => (
                        <div key={index}>
                            <h1 className="product-masthead">{category.category}</h1>
